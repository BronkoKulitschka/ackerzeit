...
(function (root) {
  ...
  function tick(s){
    ...
    for(const j of [...s.jobs]){const f=s.fields.find(f=>f.id===j.field);if(reason(s,f,j.key,true)||weatherReason(s,f,j.key))continue;const work=Math.min(hours,j.remaining);if(work<=0)continue;j.remaining=round(j.remaining-work);hours=round(hours-work);if(j.key!=='harvest'){s.tractor.hours=round(s.tractor.hours+work);s.tractor.condition=clamp(round(s.tractor.condition-work*0.16),0,100);}if(j.remaining<=0){finish(s,j,f);s.jobs=s.jobs.filter(x=>x.id!==j.id);important=true;}}
  ...
  function advance(s,count=1){let days=0;for(;days<count;){days++;if(tick(s))break;}return days;}
  ...
  function validate(s){
    ...
    for(const j of s.jobs){if(!s.fields.some(f=>f.id===j.field)||!Object.hasOwn(ACTIONS,j.key)||!num(j.id,1,s.nextJob-1)||!num(j.total,0.01,1000)||!num(j.remaining,0.01,j.total)||!j.reserved)return false;const e=estimate(s,s.fields.find(f=>f.id===j.field),j.key);for(const k of Object.keys(e))if(j.reserved[k]!==e[k])return false;if(j.total!==e.hours)return false;}
  ...
  const api={CROPS,ACTIONS,SHOP,fresh,date,month,season,estimate,reason,weatherReason,queue,cancel,advance,buy,sell,maintain,expand,land,loan,price,yieldFor,busy,stockOf,validate};
